...
for exp_experts_num in [2, 4, 6, 8, 10, 12]:
    for exp_experts_ability in [(25, 5), (50, 5), (75, 5)]:
        for alpha, beta in [(1, 1), (9, 1), (7, 3), (3, 7), (1, 9)]:
            ...
            for exp_times in [1, 2, 3, 4, 5]:
                ...
                log_file = os.path.join(root_dir, f'{alpha}-{beta}-num_expert_{num_expert}_ability_{mean}-{exp_times}.txt')
                print(
                ...
                for epoch in range(epochs):
                    ...
                    for data in tqdm(trainLoader):
                        ...
                        experts_labels = experts_labels.to(device).to(torch.int64)
                        ...
                        expert_onehot_label = torch.zeros((labels.shape[0], num_expert, num_class), dtype=torch.long).to(device)
                        # 使用 scatter_ 将对应位置的值设置为 1
                        print(expert_onehot_label.shape, experts_labels.shape)
                        expert_onehot_label.scatter_(2, experts_labels.unsqueeze(-1), 1)
                        # 保证专家预测与分类器预测结果分布一致
                        expert_onehot_label = expert_onehot_label * 10.0
                        ...
                        gamma[:, 0] = alpha  # 第一列（即第一个损失）使用 weight_first
                        gamma[:, 1:] = beta  # 后续列（即后续损失）使用 weight_rest
            ...
            file_path = f"./log/CIFAR-100-exp/result-exp-CIFAR-100-{alpha}-{beta}.txt"
            with open(file_path, 'a') as file:
                file.write(f"EXPERT NUMBER: {exp_experts_num}\n")
                file.write(f"EXPERT ABILITY: ({exp_experts_ability[0]}, {exp_experts_ability[1]})\n")
                file.write(f"Ours-{alpha}-{beta} acc: {mean_our_approach_accuracy:.8f} {mean_our_approach_accuracy_std:.8f}\n")
                file.write(f"-------------------------------------------------------------------------------\n")